...
import (
	...
	"github.com/3d0c/mininet/pkg/mn"
...
func main() {
	...
	if f, err := os.OpenFile(historyFn, os.O_RDONLY, 0644); err == nil {
		line.ReadHistory(f)
		f.Close()
	}

	defer func() {
		if f, err := os.OpenFile(historyFn, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644); err == nil {
			line.WriteHistory(f)
			f.Close()
		}
	}()
